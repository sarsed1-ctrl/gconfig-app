...
Users sign in on **imos.netshop.imos3d.com in a full browser window** (not inside an iframe).

### Why login loops in the embedded view

Chrome, Edge, and Safari block **third-party cookies** in cross-origin iframes. GConfig (`localhost` or `gconfig.online`) embeds IMOS (`3034.netshop.imos3d.com`), so IMOS session cookies are dropped after login — the catalog asks to sign in again.

**Default fix:** [`kitchen.html`](../kitchen.html) shows a launcher. Click **Open Kitchen configurator** → full-window IMOS → login works.

| URL | Behavior |
|-----|----------|
| `/kitchen.html` | Launcher (recommended) |
| `/kitchen.html?mode=redirect` | Skip launcher, open IMOS in this tab |
| `/kitchen.html?mode=embed` | Iframe only (login often fails) |
