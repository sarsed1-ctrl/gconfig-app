...
class Furniture3D {
    ...
    buildCloset(p) {
        ...
        this.buildCarcassSection(this.root, upper.w, upper.h, upper.d, T, lower.h + gap, carcassMat, edgeMat, p.backWall, backMat);

        const shelfDefs = Array.isArray(p.shelves) ? p.shelves : [];
        shelfDefs.forEach((sec) => {
            const baseY = sec.zone === 'upper' ? lower.h + gap : 0;
        ...
        const upperCenterY = lower.h + gap + upper.h / 2;
