...
function isHorizontalPanelLayout(sheet) {
    const isSide = sheet.layout === 'side' || /^side_/.test(sheet.partId || '');
    const isEdge = sheet.layout === 'edge';
    return !isSide && !isEdge && !isEdgePairSheet(sheet);
}
...
function measureDrillingSheet(sheet, opts = {}) {
    ...
    const isHorizontalLayout = isHorizontalPanelLayout(sheet);
    ...
    const skipHoleList = isProduction && (isSideLayout || isEdge || isEdgePair || isHorizontalLayout);
    ...
    if (isProduction && prodLayout) {
        ...
        } else if (isSideLayout || isEdge || isHorizontalLayout) {
...
function drawPartDrillingSheet(ctx, sheet, opts = {}) {
    ...
    const isSide = sheet.layout === 'side' || /^side_/.test(sheet.partId || '');
    const isEdge = sheet.layout === 'edge';
    const isHorizontal = isHorizontalPanelLayout(sheet);
    ...
    if (chartMode === 'production' && prod && isHorizontal) {
        const horizResult = prod.drawProductionHorizontalPanelContent(ctx, {
            sheet,
            ox,
            oy,
            pw,
            ...
            layoutScale,
            mapHoleToSheet,
            getHoleFillColor: holeFillColor,
            getHoleRadius: holeVisualRadius,
            canvasW: W,
            lang,
        });
        const legendY = (horizResult.tableLayout?.top || oy + ph)
            + (horizResult.tableLayout?.tableH || 0)
            + legendGap;
        ctx.fillStyle = '#555';
        ctx.font = `${fs(9, layoutScale)} Segoe UI, Arial, sans-serif`;
        const originText = lang === 'ru'
