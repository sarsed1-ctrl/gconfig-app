...
function computeQuote(payload, catalog, options) {
    ...
    for (const raw of items) {
        const materialCode = raw.material == null ? '' : String(raw.material).trim();
        if (!materialCode || materialCode.toLowerCase() === 'null' || materialCode === 'HDF') {
            continue;
        }
    ...
    if (!lines.length) {
        const err = new Error('No priceable line items in Tellimus file');
        err.statusCode = 422;
        throw err;
    }
