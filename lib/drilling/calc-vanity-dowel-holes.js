...
function shelfSpacingApi() {
    if (typeof require !== 'undefined') {
        try {
            return require('../shelf-spacing.js');
        } catch (e) {
            return {};
        }
    }
...
function resolveShelfPositionsFromConfig(count, shelfThickMm, spacingMm, positions) {
    ...
    const uniform = Math.max(1, parseInt(spacingMm, 10) || 150);
    const api = shelfSpacingApi();
    if (api.computeShelfBottomPositions) {
        return api.computeShelfBottomPositions(n, shelfThickMm, {
            mode: 'uniform',
            uniformSpacing: uniform,
            gaps: [],
        });
    }
    return Array.from({ length: n }, (_, i) => uniform * (i + 1));
}

function getShelfOpenIntervals(totalInnerMm, perpCount, spacingMm, shelfThickMm, perpPositions) {
    const api = shelfSpacingApi();
    if (Array.isArray(perpPositions) && perpPositions.length && api.getShelfOpenIntervalsFromPositions) {
        return api.getShelfOpenIntervalsFromPositions(totalInnerMm, perpPositions, shelfThickMm);
    }
 ...
 * @property {number[]} [positionsH]
 * @property {number[]} [positionsV]
...
function calcClosetCarcassDowelHoles(config) {
    ...
    const vPositions = resolveShelfPositionsFromConfig(shelvesV, shelfT, spacingV, positionsV);
    ...
    if (shelvesH > 0) {
        const shelfD = d - 4;
        const xSegments = getShelfOpenIntervals(innerW, shelvesV, spacingV, shelfT, vPositions);
        for (let i = 0; i < shelvesH; i++) {
            const posFromBottom = hPositions[i] ?? spacingH * (i + 1);
...
function readShelfAxisPositionsFromDom(doc, count, shelfThick, modeRadioName, uniformId, gapPrefix) {
    const api = shelfSpacingApi();
    const n = Math.max(0, Math.min(5, parseInt(count, 10) || 0));
    if (!n || !api.computeShelfBottomPositions) return [];
    const uniform = parseInt(doc.getElementById(uniformId)?.value, 10) || 150;
    const mode = doc.querySelector(`input[name="${modeRadioName}"]:checked`)?.value || 'uniform';
    const gaps = api.readGapValues ? api.readGapValues(doc, gapPrefix, n, uniform) : [];
    return api.computeShelfBottomPositions(n, shelfThick, { mode, uniformSpacing: uniform, gaps });
}
...
function buildVanityDrillingConfigFromDom(doc, labels) {
    ...
    const vanityShelvesH = num('vanityShelvesH');
    const vanityShelvesV = num('vanityShelvesV');
    const vanityShelfT = num('vanityShelfThick', carcassT);
    const vanitySpacingH = num('vanitySpacingH', 150);
    const vanitySpacingV = num('vanitySpacingV', 200);
    return {
        w1: num('w1'),
        h1: num('h1'),
        d1: num('d1'),
...
function buildUpperDrillingConfigFromDom(doc, labels) {
    const num = (id, fallback = 0) => parseInt(doc.getElementById(id)?.value, 10) || fallback;
    const carcassT = num('carcassThick', 16);
    const upperShelvesH = num('upperShelvesH');
    const upperShelvesV = num('upperShelvesV');
    const upperShelfT = num('upperShelfThick', carcassT);
    const upperSpacingH = num('upperSpacingH', 100);
    const upperSpacingV = num('upperSpacingV', 200);
    return {
        w2: num('upper_w'),
        h2: num('upper_h'),
        d2: num('upper_d'),
